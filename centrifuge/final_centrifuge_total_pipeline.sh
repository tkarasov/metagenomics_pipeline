...
qsub -hold_jid "reads.patho*" -v curr_direc=$curr_direc \
 /ebio/abt6_projects9/metagenomic_controlled/Programs/metagenomics_pipeline/centrifuge/centrifuge_total_step1_v2.sh
...
ls $curr_direc/*R1.fq > $curr_direc/centrifuge_output/metagenomic_out.txt

my_jobs=$((`wc -l $curr_direc/centrifuge_output/metagenomic_out.txt | awk '{print $1}'`))

qsub -hold_jid "run_centrifuge*" -t 1-$((my_jobs)) -v curr_direc=$curr_direc \
/ebio/abt6_projects9/metagenomic_controlled/Programs/metagenomics_pipeline/centrifuge/centrifuge_total_step2_v2.sh
