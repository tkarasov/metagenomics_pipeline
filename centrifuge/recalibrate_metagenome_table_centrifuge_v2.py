...
def convert_per_plant(meta_corrected, all_depth, genus_dict):
    ...
    for rec in meta_corrected.keys():
        new = rec #rec.split("/")[-1].split(".R1.fq.report")[0]
        # print(rec)
        meta_corrected_new[new] = meta_corrected[rec] / all_depth[new]

    return meta_corrected_new
    #pd.DataFrame.from_dict(meta_corrected_new)
...
###########################################################################
# Establish files and parameters
###########################################################################
...
metagenome_data.index = metagenome_data['family']
metagenome_data = metagenome_data.drop('family', axis=1)
...
###########################################################################
# Get information on genome size per family
###########################################################################
...
renamed = [rec.split(".")[0] for rec in meta_family_correct.columns]
meta_family_correct.columns = renamed
...
###########################################################################
# Correct the metagenome table with the info from previous sections
###########################################################################
...
meta_family_per_plant.to_csv("meta_family_corrected_per_plant_v2_" + organism + ".csv", header=True, index=True)

print(organism, str(family_unaccounted))


#-host $curr_direc -org bacteria -resize 3870000 -min_recal 3870000
