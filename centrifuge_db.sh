...
for file in `ls | grep R1R2.fq.gz`; do full_dir=`pwd`;
echo -e "1\t"$full_dir/$file"\t"None"\t" $full_dir/centrifuge_output/$file.out"\t"$full_dir/centrifuge_output/$file.report >> $full_dir/centrifuge_output/all_fastq_unpaired ; done
